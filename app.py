...
pip install flask reportlab openpyxl gspread google-auth gunicorn
"""
...
def siguiente_recibo_sheets(dni):
    _, sh = get_sheets_client()
    if not sh:
    ...
    try:
        ...
        print(f"Error contador sheets: {e}")
        return siguiente_recibo_local(dni)

def recibo_actual_sheets(dni):
    _, sh = get_sheets_client()
    if not sh:
...
def dibujar_planilla(c, datos, ox, oy, ancho, alto):
    ...
    yb = oy - alto
    c.setStrokeColor(colors.HexColor(AZ))
    c.setLineWidth(1)
    ...
    t("RECIBO N°", sep2 + 0.2*cm, y_cab + cab_h*0.65,
      sz=6.5, col=colors.HexColor(TXT2))
    ...
    for lb, i in [("N° / FECHA", 0), ("COD.\nCC", 1), ("CENTRO DE\nCOSTO", 2),
        ...
        for j, p in enumerate(lines):
            t(p, cx, cy + off - j*0.22*cm, sz=6, bold=True,
              al="center", col=colors.white)
    ...
    for i, lb in {3: "HORA", 4: "PUNTO PARTIDA",
                  5: "HORA", 6: "PUNTO DE LLEGADA"}.items():
    ...
    for idx in range(n_rows):
        ...
        ln(ox, y_r - fh_row, ox + ancho, y_r - fh_row,
           g=0.3, col=colors.HexColor(BORDER))
        for i in range(1, len(cw)):
            ln(xc(i), y_r - fh_row, xc(i), y_r,
               g=0.3, col=colors.HexColor(BORDER))
        if idx < len(datos["filas"]):
            ...
            vals = [
                ...
                f.get("detalle",""),
                f"{float(f.get('importe', 0)):.2f}"
            ]
            for i, v in enumerate(vals):
                al = ("right"  if i == 8 else
                      "center" if i in (0, 3, 5) else "left")
                px = (xc(i) + cw[i] - 0.1*cm if al == "right" else
                      xc(i) + cw[i]/2         if al == "center" else
                      xc(i) + 0.12*cm)
    ...
    for tr in ["TAXI", "OMNIBUS", "COLECTIVO", "OTROS"]:
        ...
        if sel == tr:
            t("X", tx + 0.02*cm, y_tot - obs_h*0.72,
              sz=7.5, bold=True, col=colors.HexColor(AZ))
    ...
    firma_h = alto * 0.22
    fw      = ancho / 3
    ln(ox, y_obs - firma_h, ox + ancho, y_obs - firma_h,
       g=0.8, col=colors.HexColor(AZ))
    ...
    for i, (titulo, cargo_f, nombre, fimg_b64) in enumerate(firmantes):
        fx = ox + i * fw
        if i > 0:
            ln(fx, y_obs - firma_h, fx, y_obs,
               g=0.4, col=colors.HexColor(BORDER))
        t(titulo, fx + 0.3*cm, y_obs - firma_h*0.18,
          sz=7, bold=True, col=colors.HexColor(AZ))
        ...
        ln(fx + 0.3*cm, linea_y, fx + fw - 0.3*cm, linea_y,
           g=0.5, col=colors.HexColor("#888888"))
        t(cargo_f, fx + 0.3*cm, linea_y - firma_h*0.18,
          sz=6.5, col=colors.HexColor(TXT2))
        if nombre:
